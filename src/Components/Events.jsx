...
const Events = () => {
    const [events,setevents]=useState([ {id:101,name:"event1",date:"10-6-2024",students:[]},
                                        {id:102,name:"event1",date:"10-4-2024",students:[]},
                                        {id:103,name:"event1",date:"10-4-2024",students:[]}]);
                    
    const [student,setStudent]=useState({id:"",name:"",eventid:"",gender:"",sem:""});
  ...
  return (
    <>  <h1>Event Registration Form</h1>
        ...
        <ul>
            ...
            events.map((e,index)=>{
                return(
                <li key={index}>{e.id}  {e.name}  {e.date}  {' '}
                {e.students?.map((student,index)=>{
                    return(
                        <p key={index}> {student.id} {student.name} {student.gender} {student.sem}</p>
                    )
                })}</li>
                )
            })
        ...
        {/* <form action="" onSubmit={handleSubmit} align="center"> */}
            ...
            <label >Student ID:
                <input type="number" name="id" value={student.id} onChange={handleInput} autoFocus/>
            </label>
            <br />
            <br />
            <label >Student Name: 
            ...
            <br />
            <label >Event ID:
                <input type="number" name="eventid" value={student.eventid} onChange={handleInput} />
            </label>
            <br />
            <br />
            Gender:
            ...
            <label htmlFor="">
                Male:
                <input type="radio" value="male" name="gender" onChange={handleInput}/>
            </label>
            <br />
            <label htmlFor="">
                Female:
                <input type="radio" value="female"  name="gender" onChange={handleInput}/>
            </label>
            <br />
            <br />
        ...
        <form action="" onSubmit={eventSubmit}>
            <h1>Add event</h1>
            <label >Event ID:
                <input type="number" name="id" value={e1.id} onChange={handleE1} autoFocus/>
            </label>
            <br />
            <br />
            <label >Event Name: 
            ...
            <ul>
                {events.map((e,index) => {
                    if(e.students.length > 0){
                        return(
                            <li key={index}>{e.id}  {e.name}  {e.date}  {' '}</li>
                        )
                    }
                })}
            </ul>
